 ...
 * 解释：container3 这个对象，thread-1,thread-2都需要申请到这个对象才能进行自己线程里面的逻辑
 * 在thread-2 使用wait()，之后，会释放 container3 这个对象锁，thread-1拿到之后进行add 方法，当size=5之后
 * thread-1使用notify()唤醒 thread-2,但是 thread-2要执行下去也需要  container3 对象锁，此时thread-1 还在占用，没办法申请
        ...
        new Thread(()->{
            ...
            synchronized (container3){
                ...
                if (container3.size()!=5){
                    try {
                        System.out.println("thread-2:"+container3.size());
                        container3.wait();// 使用notify()只后，此线程重新申请获得container3这个对象锁，进行下面的代码
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    System.out.println("thread-2 开始继续跑");
                }
                System.out.println("thread-2 结束"+(container3.size()));
            }
